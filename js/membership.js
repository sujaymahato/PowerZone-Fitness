...
document.getElementById('basicSubmitBtn').addEventListener('click', function() {
    ...
    // Check if all the input fields are filled (except payment method and duration)
    if (!name || !email || !phone) {
    ...
    if (!duration) {
        ...
        messageDiv.style.display = 'block';
        return; // Exit if duration is not selected
    }

    // Check if payment method is selected after fields and duration are filled
    if (!(cardChecked || paypalChecked)) {
        messageDiv.innerText = "Please select a payment method.";
        messageDiv.style.display = 'block';
        return; // Exit if payment method is not selected
    }

    // If all fields, duration, and payment method are filled, proceed
    ...
    setTimeout(() => {
        document.getElementById('mainMessage').style.display = 'none'; // Hide the message when modal is closed
    }, 4000);
});
